...
export default function PartnerTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/partner/tasks${params.toString() ? `?${params.toString()}` : ''}`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (selectedTask?.id === taskId) {
          setSelectedTask(prev => prev ? { ...prev, status, completed_at: status === 'done' ? new Date().toISOString() : undefined } : null);
        }
      } else {
        const data = await response.json().catch(() => ({}));
      ...
      <Dialog open={!!selectedTask} onOpenChange={(open: boolean) => { if (!open) setSelectedTask(null); }}>
        <DialogContent className="max-w-lg">
          {selectedTask && (
            <>
              <DialogHeader>
                <DialogTitle className={selectedTask.status === 'done' ? 'line-through text-muted-foreground' : ''}>
                  {selectedTask.title}
                </DialogTitle>
