...
export default function PartnerV2AddStudentPage() {
  ...
  const handleSubmit = async (action: 'save' | 'save_apply' = 'save') => {
    ...
    try {
      ...
      const response = await fetch('/api/partner/students', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (action === 'save_apply') {
          router.push(`/partner-v2/applications/new?student=${data.student.id}`);
        } else {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Add a student record. They can claim their account later.</p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <div className="flex items-center justify-end gap-3 mt-8 pt-6 border-t">
            ...
            <Button onClick={() => handleSubmit('save')} disabled={isSubmitting} className="gap-2">
              {isSubmitting ? <IconLoader2 className="h-4 w-4 animate-spin" /> : <IconCheck className="h-4 w-4" />}
              Save Student
            </Button>
            <Button onClick={() => handleSubmit('save_apply')} disabled={isSubmitting}>
